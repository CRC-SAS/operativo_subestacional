...
#########################
# Percentil 50+
fcst_m, hcst_f, media_f, pctil_f = get_data(fecha_d, 50, variable, modelo)
...
p_down50_corr, p_up50_corr = calc_prob_corr(p_down50, p_up50, variable, 50)
#p1_80_corr, p2_corr = calc_prob_corr(p1_80, p2, variable, 80)
...
for percentil in ['50-', '50+']:
    ...
    # Fechas
    f1s = [fecha_d, fecha_d+dt.timedelta(days=7), fecha_d+dt.timedelta(days=14)]
    f2s = [fecha_d+dt.timedelta(days=6), fecha_d+dt.timedelta(days=13), fecha_d+dt.timedelta(days=27)]

    for week, f1, f2 in zip([1,2,3], f1s, f2s):
        print('######### Figura semana:', week)
        if percentil == '50-':
            mapa_probabilidad(variable, p_down50_corr, percentil, week, f1, f2, c_out_f, corr=True)
            mapa_probabilidad(variable, p_down50, percentil, week, f1, f2, c_out_f, corr=False)
        elif percentil == '50+':
            mapa_probabilidad(variable, p_up50_corr, percentil, week, f1, f2, c_out_f, corr=True)
